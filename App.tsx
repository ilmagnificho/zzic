      ...
      <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
        <div className="w-full max-w-sm bg-zinc-900 rounded-3xl border border-zinc-800 p-8 relative shadow-2xl">
          ...
          <p className="text-center text-zinc-500 text-xs mb-8">로그인하면 기록이 영구 저장됩니다.</p>
           ...
           {/* GUEST MODE HIGHLIGHT (Lower Barrier) */}
           <button 
              onClick={handleGuest}
  ...
  const CommentItem: React.FC<{ comment: Comment, depth?: number, allComments: Comment[] }> = ({ comment, depth = 0, allComments }) => {
    ...
    return (
        <div className={`animate-in fade-in slide-in-from-bottom-2 duration-300 ${depth > 0 ? 'mt-2' : 'mt-4 border-b border-zinc-900/50 pb-4 last:border-0'}`}>
            ...
            {replies.length > 0 && <div className="pl-8">{replies.map(reply => <CommentItem key={reply.id} comment={reply} depth={depth + 1} allComments={allComments} />)}</div>}
        </div>
    );
  };
    ...
    return (
        <div className="pb-24 lg:pb-0 min-h-screen animate-in slide-in-from-right duration-300">
            ...
            <div className="px-5 pt-6">
                ...
                <div className="pb-10">
                    <h3 className="text-sm font-black text-white mb-4 flex items-center gap-2 uppercase tracking-wide"><MessageSquare size={16} className="text-zzic"/> {t('detail_discussion')} <span className="text-xs text-zinc-500 ml-auto font-normal">{t('detail_discussion_count').replace('{0}', comments.filter(c => c.marketId === activeMarketId).length.toString())}</span></h3>
                    ...
                    <div className="space-y-0">
                        {comments.filter(c => c.marketId === activeMarketId && !c.parentId).length === 0 && <div className="text-center py-8 text-zinc-600 text-xs font-bold">{t('detail_no_comments')}</div>}
                        {comments.filter(c => c.marketId === activeMarketId && !c.parentId).map(comment => <CommentItem key={comment.id} comment={comment} allComments={comments.filter(c => c.marketId === activeMarketId)} />)}
                    </div>
                </div>
            </div>
             <div className="py-8 text-center px-4 lg:hidden"><p className="text-[10px] text-zinc-600 font-medium leading-relaxed">{t('footer_text')}</p></div>
